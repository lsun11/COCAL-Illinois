subroutine IO_output_plot_xyz_qeos
  use phys_constant, only : long
  use def_metric, only : psi, alph, bvxd, bvyd, bvzd
  use def_matter, only : rhof, omef
...
! --- For fluid plot
  ...
  do irf = nrf, 0, -1
    write(12,'(1p,6e20.12)') -rg(irf), rhof(irf,ntgeq,npgxzm), &
    &                                 omef(irf,ntgeq,npgxzm)
  end do
  do irf = 0, nrf
    write(12,'(1p,6e20.12)')  rg(irf), rhof(irf,ntgeq,npgxzp), &
    &                                 omef(irf,ntgeq,npgxzm)
!
  end do
  close(12)
!
end subroutine IO_output_plot_xyz_qeos
