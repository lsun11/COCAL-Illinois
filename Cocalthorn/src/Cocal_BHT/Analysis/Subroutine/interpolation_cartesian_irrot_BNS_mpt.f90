subroutine interpolation_cartesian_irrot_BNS_mpt(impt)
  ...
  real(long) :: par, dxvep,dyvep,dzvep,psif4
  ...
  do ip = 0, npf
    do it = 0, ntf
      do ir = 0, nrf
        ...
        vxu(ir,it,ip) = vepxf(ir,it,ip)/psif4*par
        vyu(ir,it,ip) = vepyf(ir,it,ip)/psif4*par
        vzu(ir,it,ip) = vepzf(ir,it,ip)/psif4
      end do
    end do
  end do
...
!
end subroutine interpolation_cartesian_irrot_BNS_mpt
