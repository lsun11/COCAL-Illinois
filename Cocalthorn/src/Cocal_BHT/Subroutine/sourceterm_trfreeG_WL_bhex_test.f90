...
!    fnc0(0:nrg,0:ntg,0:npg) = Lij_grid(0:nrg,0:ntg,0:npg,ia,ib)  
...
!!fail     if (chgra == 'k') tfliep4aij = tfaij*bepadpsi + ps4oal*tfliebaij - sou_daij
!!fail     sou_qij(irg,itg,ipg,ic) = ps4oal*tfliebaij 

!          The following converges (H2.0r resolution)
!converges if (chgra == 'k') tfliep4aij = tfaij*bepadpsi + ps4oal*(-sgam*trliebaij) 


!          Conversion of dAij into volume integral over dGreen. Same problem.
!          if (chgra == 'k') then
!
!            tfliebaij = aijdb(ia,ib) - sgam*trliebaij    ! without daij, but included in the trace
!
!            bda = bvxgc*daalph(1) + bvygc*daalph(2) + bvzgc*daalph(3)
!            bdp = bvxgc*dapsi(1)  + bvygc*dapsi(2)  + bvzgc*dapsi(3)
!
!            tfliep4aij = tfaij*bepadpsi + ps4oal*tfliebaij   &
!                   &   + ps4oal*aij(ia,ib)*(alpgcinv*bda - 4.0d0*psiinv*bdp - cdivbc) 
!          end if
...
!
!          tfliep4aij = tfliep4aij*zerofac  ! test1 converges
!          tfr2dab = tfr2dab*zerofac        ! test2 blows up
!          tfrnlab = tfrnlab*zerofac        ! test3 blows up
!          tfrpddal = tfrpddal*zerofac      ! test4 blows up
!          tfaij = tfaij*zerofac            ! test5 blows up
!          tfaa = tfaa*zerofac              ! test6 blows up
!          gamdhdh = gamdhdh*zerofac
...
!          if(itg.eq.ntg/2.and.ipg.eq.1.and.ic.eq.1) &
!          write(6,*) souten(irg,itg,ipg,ic) , &
!             &      tfheli , tfliep4aij &
!             &            , tfrnlab , tfrpddal , tfaa &
!             &            , gamdhdh , ogdphdph
!
!
        end do
...
!        soubi(irg,itg,ipg,1) = -2.0d0*psigc4*br/alpgc
!        soubi(irg,itg,ipg,2) = -2.0d0*psigc4*bth/alpgc
!        soubi(irg,itg,ipg,3) = -2.0d0*psigc4*bph/alpgc
      end do
    end do
  end do
!
