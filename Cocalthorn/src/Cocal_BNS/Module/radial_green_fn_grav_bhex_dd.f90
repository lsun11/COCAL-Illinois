...
module radial_green_fn_grav_bhex_dd
  ...
  real(long), pointer  ::  hdrgfn_dd(:,:,:)
contains
subroutine allocate_hgfn_bhex_dd
  implicit none
  call alloc_array3d(hgfn_dd,1,nrg,0,nlg,0,nrg)
  call alloc_array3d(gfnsf_dd,0,nlg,0,nrg,1,4)
  call alloc_array3d(hdrgfn_dd,1,nrg,0,nlg,0,nrg)
end subroutine allocate_hgfn_bhex_dd
! ----------------------------------------------
! Subroutine
...
!         partial_r' g_l(r,r')
          hdrgfn_dd(irr,il,ir) = fac1*fac2 &
          &   *(bhradinv*dble(il)*(hrg(irr)*bhradinv)**(il-1) + &
          &     bhradinv*dble(il+1)*(bhrad*hrginv(irr))**(il+2)) &
          &   *((radext*rginv(ir))**(il+1) - (rg(ir)*radextinv)**il)
        end do
      end if
...
!         partial_r' g_l(r,r')
          hdrgfn_dd(irr,il,ir) = -fac1*fac2 &
          &   *((rg(ir)*bhradinv)**il - (bhrad*rginv(ir))**(il+1)) &
          &   *(radextinv*dble(il+1)*(radext*hrginv(irr))**(il+2) + &
          &     radextinv*dble(il)*(hrg(irr)*radextinv)**(il-1))
        end do
      end if
    end do
  end do
...
!
end subroutine calc_hgfn_bhex_dd
end module radial_green_fn_grav_bhex_dd
