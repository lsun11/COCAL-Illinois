subroutine bh_boundary_dh_psi_test(sou_surf)
...
!  real(long), pointer :: sou_surf(:,:), dsou_surf(:,:)
  ...
  integer    :: itg, ipg
!
! The factor 0.8 below is to make alpha positive 
...
!      dr2dr1 = (rad1 - sepa*st*cp)/rad2
      sou_surf(itg,ipg) = 1.0d0+0.5d0*bhmass/rad1+0.5d0*bhmass/rad2
    end do
  end do
!
end subroutine bh_boundary_dh_psi_test
