subroutine update_parameter_axisym_WL_peos(convf)
...
!
  ...
  gout = 1.0d0+hyyd(nrf,ntgxy,0)
! write (6,*) "test values"
! write (6,'(1p,3e12.4)') ome, ber, radi
! write (6,'(1p,3e12.4)') psi(nin,0,0),psi(0,0,0),psi(nrf,ntg/2,0)
! write (6,'(1p,3e12.4)') alph(nin,0,0),alph(0,0,0),alph(nrf,ntg/2,0)
! write (6,'(1p,3e12.4)') error
! write (6,'(1p,3e12.4)') pain, pamid, paout
! write (6,'(1p,3e12.4)') gin, gmid, gout
! ################################################################ 
  do
...
!      ov2out = ovout**2+bvxd(nrf,ntg/2,0)**2+bvzd(nrf,ntg/2,0)**2
    ov2in  = gin*ovin**2 
    ov2mid = gmid*ovmid**2
    ov2out = gout*ovout**2
...
!    test
!if (numero.eq.1) then
! write (6,*) + loghc, radi**2, aout, + 0.5d0*dlog(termout), &
!    &  - dlog(ber), ov2out
...
!!write (6,*) "h  = ", hh
!
end subroutine update_parameter_axisym_WL_peos
