...
!
  ...
  real(long)     ::    loghc, loghs
  real(long)     ::    radiold, berold, omeold
  real(long)     ::    ddradi, ddber, ddome
  real(long)     ::    facfac, numero, error, hc, pre, rho, ene, hs, dummy
...
!
  ...
  gg  = 0.0d0
!
  call quark_rho2phenedpdrho(rhoc_qs, pre, hc, ene, dummy)
  loghc = dlog(hc)
  call quark_rho2phenedpdrho(rhos_qs, pre, hs, ene, dummy)
  loghs = dlog(hs)
  call search_rhofmax_xaxis_grid(nmx)
!  nmx = 0
!testtest
...
!
    sgg(1) = -(radi**2*ain  + 0.5d0*dlog(termin)  + jint_in  - dlog(ber) + loghs)
    sgg(2) = -(radi**2*aout + 0.5d0*dlog(termout) + jint_out - dlog(ber) + loghs)
