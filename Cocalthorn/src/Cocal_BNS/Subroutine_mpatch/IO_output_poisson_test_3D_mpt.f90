subroutine IO_output_poisson_test_3D_mpt(impt)
  ...
  implicit none
  integer :: ir, it, ip, impt
  character(len=1) :: np(5) = (/'1', '2','3', '4', '5'/)
!
! --- Matter
  open(12,file='rnsflu_3D_mpt'//np(impt)//'.las',status='unknown')
...
!
! --- Metric potentials.
  open(13,file='rnsgra_3D_mpt'//np(impt)//'.las',status='unknown')
...
!
! --- Coordinate grids.
  open(14,file='rnsgrids_3D_mpt'//np(impt)//'.las',status='unknown')
  ...
  close(14)
!
end subroutine IO_output_poisson_test_3D_mpt
