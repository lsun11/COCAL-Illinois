    ...
    if (char_mp.eq.'bvxd'.or.char_mp.eq.'bvyd'.or.char_mp.eq.'bvzd') then  
      ...
      do ipg = 1, npg
        do itg = 1, ntg
          ...
          sou_surf(itg,ipg) = - ome_bh*vphi_cm - spin_bh*vphi_cbh
...
!
if (bh_bctype.eq.'AH'.or.bh_bctype.eq.'PG') then 
!
...
!      sou_surf(1:ntg,1:npg) = alph_bh*sou_surf(1:ntg,1:npg)
      sou_surf(1:ntg,1:npg) = alph_bh*psigc
...
!    
    do ipg = 1, npg
      do itg = 1, ntg
        ...
        vphi_cm  = hvec_bh_cm_phig(itg,ipg,ii)
        vphi_cbh = hvec_bh_cbh_spin(itg,ipg,ii)
        if (bh_bctype.eq.'PG') then 
          vphi_cm = 0.0d0
          if (char_mp.eq.'bvyd') vphi_cm = - dis
        end if
