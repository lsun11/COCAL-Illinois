subroutine calc_proper_mass_peos_irrot
  use phys_constant, only  :   long, pi
  use grid_parameter, only :   nrf, ntf, npf
  use def_matter_parameter, only : radi
  use make_array_3d
  use def_quantities, only : propermass
  use interface_source_proper_mass_peos_irrot
...
!      
  call source_proper_mass_peos_irrot(souf)
...
!
  deallocate(souf)
end subroutine calc_proper_mass_peos_irrot
