subroutine source_virial_gravity_WL(sou_Wgra)
  ...
  real(long) :: pi4inv
  real(long) :: alpgc, psigc, bvxgc, bvygc, bvzgc, gamu(3,3), &
  &             hhxxu, hhxyu, hhxzu, hhyxu, hhyyu, hhyzu, &
  &             hhzxu, hhzyu, hhzzu, ric(3,3), ricsc, &
  &             psigc2, psigc6, alp2inv, psal2inv, aijaij, trkgc, &
  &             dapsi(3), daalph(3), dpsi2, dalph2, bvdal, fac23
...
!
        ...
        do ib = 1, 3
          do ia = 1, 3
            dpsi2  = dpsi2 + gamu(ia,ib)*dapsi( ia)*dapsi( ib)
            dalph2 = dalph2+ gamu(ia,ib)*daalph(ia)*daalph(ib)
            ricsc  = ricsc + gamu(ia,ib)*ric(ia,ib)
          end do
        end do
        bvdal = bvxgc*daalph(1) +  bvygc*daalph(2) +  bvzgc*daalph(3)
!
        sou_Wgra(irg,itg,ipg) = pi4inv*(2.0d0*dpsi2 - psal2inv*dalph2 &
        &                     + 0.75d0*psigc6*(aijaij - fac23*trkgc**2) &
        &                     + psigc6*alp2inv*trkgc*bvdal + 0.25*ricsc*psigc2)
!
      end do
    end do
  end do
