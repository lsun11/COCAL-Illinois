...
!    &         /(pain**(2.d0*radi**2)*vphiyin**2 &
!    &         + A2DR*index_DR*termin &
!    &         + 2.0d0*pain**(2.d0*radi**2)*vphiyin*ome**2*ovin*jin &
!    &         - ome*termin*jin)
!    dodoc_mid = A2DR*index_DR*termmid*(ome/(omemid+small))**(index_DR-1.0d0) &
!    &         /(pamid**(2.d0*radi**2)*vphiymid**2 &
!    &         + A2DR*index_DR*termmid*(ome/(omemid+small))**index_DR &
!    &         + 2.0d0*pamid**(2.d0*radi**2)*vphiymid*omemid**2*ovmid*jmid &
!    &         - omemid*termmid*jmid)
!    dodoc_out = A2DR*index_DR*termout*(ome/(omeout+small))**(index_DR-1.0d0) &
!    &         /(paout**(2.d0*radi**2)*vphiyout**2 &
!    &         + A2DR*index_DR*termout*(ome/(omeout+small))**index_DR &
!    &         + 2.0d0*paout**(2.d0*radi**2)*vphiyout*omeout**2*ovout*jout &
!    &         - omeout*termout*jout)
!
    dodoc_in  = -((A2DR*index_DR*omein*termin) &
    &         /(ome*(-(A2DR*index_DR*termin) &
    &         +(jin*termin)/omein  &
    &         - 2.0d0*jin*ovin*pain*vphiyin - pain*vphiyin**2)))
    dodoc_mid = -((A2DR*index_DR*(ome/omemid)**index_DR*omemid*termmid) &
    &         /(ome*(-(A2DR*index_DR*(ome/omemid)**index_DR*termmid) &
    &         +(jmid*termmid)/omemid &
    &         - 2.0d0*jmid*ovmid*pamid*vphiymid - pamid*vphiymid**2)))
    dodoc_out = -((A2DR*index_DR*(ome/omeout)**index_DR*omeout*termout) &
    &         /(ome*(-(A2DR*index_DR*(ome/omeout)**index_DR*termout) &
    &         +(jout*termout)/omeout &
    &         - 2.0d0*jout*ovout*paout*vphiyout - paout*vphiyout**2)))
!
!!write(6,*) 'dodoc'
!!write(6,*) ome,omemid,omeout
!!write(6,*) dodoc_in, dodoc_mid, dodoc_out
