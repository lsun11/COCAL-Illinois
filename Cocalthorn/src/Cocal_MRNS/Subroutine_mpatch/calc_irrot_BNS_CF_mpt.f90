subroutine calc_irrot_BNS_CF_mpt(total_iteration)
  ...
  real(long) :: xa,xb,fa,fb,delta,rm_eps,f1,f2, iter_eps
...
!
  ...
  iter_eps = 1.0d-00
  call iter_irrot_BNS_CF_mpt(iter_count,1, iter_eps)     !  1 for freezing hydro in first 5 iterations
...
!  call copy_def_quantities_BNS_from_mpt(nmpt)
!  fa = (admmass_asymp - 2.7d0)/2.7d0
!  write(6,*)  "xa,fa=", xa, fa,  "            admmass_asymp=", admmass_asymp
  call chdir('../')
!
...
!  xb = xa - 0.25d0*xa
  ...
  iter_eps = 1.0d-00
  call iter_irrot_BNS_CF_mpt(iter_count,2, iter_eps)     ! 2 to compute hydro from first iteration
  total_iteration = total_iteration + iter_count
  write(6,'(a21,i5)') '--- total iteration =', total_iteration
  call calc_physical_quantities_irrot_BNS_CF_mpt
  call write_last_physq_BNS_mpt
...
!  call copy_def_quantities_BNS_from_mpt(nmpt)
!  fb = (admmass_asymp - 2.7d0)/2.7d0
!  write(6,*)  "xb,fb=", xb, fb,  "           admmass_asymp= ", admmass_asymp
  call chdir('../')
!
...
!
  ...
  do i = 2,Nmax
    ...
    if ( dabs(delta) < rm_eps ) then
      ...
      call copy_def_matter_irrot_from_mpt(1)
...
!    write (6,'(a7,i2,a20,e20.12)') 'cycle =', icycle, '        New emdc  = ',emdc
    call iter_irrot_BNS_CF_mpt(iter_count,icycle,iter_eps)
    total_iteration = total_iteration + iter_count
    write(6,'(a21,i5)') '--- total iteration =', total_iteration
    call calc_physical_quantities_irrot_BNS_CF_mpt
    call write_last_physq_BNS_mpt
...
!    call copy_def_quantities_BNS_from_mpt(nmpt)
!    fa = (admmass_asymp - 2.7d0)/2.7d0
!    write(6,'(a30,i2,1p,2e20.12)')  '(cycle,emdc,(Madm-2.7)/2.7)=',icycle,xa,fa
    call chdir('../')
!
  end do
  write(6,*) 'No convergence after ',Nmax,' iterations.'

end subroutine calc_irrot_BNS_CF_mpt
