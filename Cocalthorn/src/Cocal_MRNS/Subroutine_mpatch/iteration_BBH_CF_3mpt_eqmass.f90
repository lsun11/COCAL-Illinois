subroutine iteration_BBH_CF_3mpt(iter_count)
  ...
  integer    :: iter_count, iter_omega = -1, &
  &             flag_tmp = 0, flag_omega = 99, &
...
! -- For ciruclar solutions
    ...
    if ((bh_soltype.eq.'CI'.or.bh_soltype.eq.'SQ').and. &
         error_tmp.le.1.0d-02) then 
!    flag_tmp = max(flag_psi,flag_alph,flag_bvxd,flag_bvyd,flag_bvzd)
!    if ((bh_soltype.eq.'CI'.or.bh_soltype.eq.'SQ').and.flag_tmp.eq.0) then 
      iter_omega = mod(iter_omega+1,2)
      if (iter_omega.eq.0) then
        impt = nmpt
        call copy_from_mpatch_all_BBH_CF(impt)
        call copy_def_metric_from_mpt(impt)
        call calc_vector_x_grav(2)
...
!
        ...
        call copy_def_quantities_to_mpt(nmpt)
        call adjust_omega_BBH(flag_omega,error_ome)
!        call adjust_omega_BBH_bisection(flag_omega,error_ome)
        ome_bh_new = ome_bh
        do impt = 1, nmpt
...
!
    if ((flag_omega==0).and.(flag_psi==0).and.(flag_alph==0).and. &
