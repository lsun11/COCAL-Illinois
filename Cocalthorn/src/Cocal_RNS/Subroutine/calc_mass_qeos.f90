subroutine calc_mass_qeos
  use phys_constant, only  :   long, pi
  use grid_parameter, only :   nrg, ntg, npg, nrf, ntf, npf
  use def_matter_parameter, only : radi
  use make_array_3d
  use def_quantities, only : admmass, komarmass, komarmass_nc
  use interface_source_adm_mass_qeos
  use interface_source_komar_mass_qeos
  use interface_source_komar_mass_compact_qeos
...
!      
  call source_adm_mass_qeos(soug,souf)
...
!
  call source_komar_mass_qeos(soug,souf)
  call vol_int_grav(soug,volg) 
  call vol_int_fluid(souf,volf)
!
  fac4pi = 0.25d0/pi
...
!
  call source_komar_mass_compact_qeos(souf)
  call vol_int_fluid(souf,volf)
!
