subroutine source_AHarea_WL_ks(sousg)
...
!      if (ipg==0) write(6,*)  "ahz(itg,ipg), ieh, rg(irg), rg(irg+1)", ahz(itg,ipg), ieh, rg(ieh),rg(ieh+1)
...
!      call interpo_linear_surface_type0(gmzzd,hzzd,irg,itg,ipg)
      ...
      sinph12 = hcosphig(ipg)*dsin(phi_spin_bh) - hsinphig(ipg)*dcos(phi_spin_bh)

      cosph12 = hcosphig(ipg)*dcos(phi_spin_bh) + hsinphig(ipg)*dsin(phi_spin_bh)

      cosang  = hsinthg(itg)*dsin(th_spin_bh)*cosph12 + hcosthg(itg)*dcos(th_spin_bh)

      rc = dsqrt(r2*r2pa2/(r2+a2*cosang**2))
   
      dcosangdth = hcosthg(itg)*dsin(th_spin_bh)*cosph12 - hsinthg(itg)*dcos(th_spin_bh)

      dcosangdph = hsinthg(itg)*dsin(th_spin_bh)*sinph12 
     
      drcdth = -a2*rc**3*cosang*dcosangdth/r2/r2pa2

      drcdph = -a2*rc**3*cosang*dcosangdph/r2/r2pa2
...
!
  ...
  deallocate(hzz)
!
end subroutine source_AHarea_WL_ks
