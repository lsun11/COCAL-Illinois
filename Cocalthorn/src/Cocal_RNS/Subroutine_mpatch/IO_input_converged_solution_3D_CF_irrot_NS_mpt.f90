subroutine IO_input_converged_solution_3D_CF_irrot_NS_mpt(impt)
...
! --- Matter
  if (impt==1 .or. impt==2) then
    open(12,file='bnsflu_3D_mpt'//np(impt)//'.las',status='old')
    ...
    open(15,file='bnssur_3D_mpt'//np(impt)//'.las',status='old')
...
!
! --- Metric potentials.
  open(13,file='bnsgra_3D_mpt'//np(impt)//'.las',status='old')
...
!
! --- Coordinate grids.
  open(14,file='bnsgrids_3D_mpt'//np(impt)//'.las',status='old')
  ...
  close(14)
!
end subroutine IO_input_converged_solution_3D_CF_irrot_NS_mpt
