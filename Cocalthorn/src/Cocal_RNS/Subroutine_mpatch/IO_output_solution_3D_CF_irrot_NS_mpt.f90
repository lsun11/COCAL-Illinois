subroutine IO_output_solution_3D_CF_irrot_NS_mpt(impt,csol)
...
! --- Matter
  if (impt==1 .or. impt==2) then
    ...
    do ip = 0, npf
      do it = 0, ntf
        do ir = 0, nrf
          write(12,'(1p,6e20.12)') emd(ir,it,ip), vep(ir,it,ip)
        end do
      end do
    end do
...
! --- Coordinate grids.
  ...
  close(14)
!
end subroutine IO_output_solution_3D_CF_irrot_NS_mpt
