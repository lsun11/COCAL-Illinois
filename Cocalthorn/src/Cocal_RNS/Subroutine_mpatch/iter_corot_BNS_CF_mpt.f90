subroutine iter_corot_BNS_CF_mpt(iter_count, iseq, iter_eps)
...
!
  ...
  use def_quantities, only : q_c
  use interface_interpo_fl2gr
  use interface_sourceterm_HaC_CF_peos_corot
  use interface_sourceterm_HaC_CF
  use interface_sourceterm_trG_CF_peos_corot
  use interface_sourceterm_trG_CF_helical
  use interface_sourceterm_MoC_CF_with_divshift_peos
  ...
  use interface_update_parameter_BNS
...
!  use interface_error_metric_type2
  use interface_error_metric_type3_mpt
  implicit none
...
!  real(long), pointer :: sou_bhsurf(:,:), dsou_bhsurf(:,:)
  ...
  &             error_emd_last, error_psi_last, error_alph_last
...
!    iter_extra = iter_extra + 1
    ...
    write(6,'(a53,i4,a53)')   "------------------------------------------- Iteration", iter_count, &
                         &    " : Errors -------------------------------------------"
!
    do impt = 1, nmpt
      write(6,'(a9,i2,a8)', ADVANCE = "NO")  " Patch # ", impt, ""
      write(6,*)  " "
...
! ---
      ...
      if (impt==1 .or. impt==2) then
        ...
        if(impt==10) then
          if (mod(iter_count,1)==0 ) then
!          if (iter_count==1 .or. iter_count==5) then
            write(char1, '(i5)') iter_count
            char2 = adjustl(char1)
            write(char4, '(i5)') impt
            char5 = adjustl(char4)
            char3 = 'before' // trim(char2) // '_mpt' // trim(char5) // '.txt'
            call printout_axis_BNS_mpt(impt,char3)
          endif
...
!          if (iter_count==1 .or. iter_count==5) then
            ...
            write(char4, '(i5)') impt
            char5 = adjustl(char4)
            char3 = 'after' // trim(char2) // '_mpt' // trim(char5) // '.txt'
            call printout_axis_BNS_mpt(impt,char3)
          endif
        end if  !  ====> iteration.txt
!        
...
! --- psi
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_HaC_CF_peos_corot(sou)
      else
...
!        call outer_boundary_d_BBH_CF(sou_outsurf,'psi ')
      ...
      if ((impt==1 .or. impt==2) .and. (iter_count>5 .or. iseq>1 .or. indata_type.eq.'3D')) &
          &     call update_parameter_BNS(conv_den)
...
!!!      call error_metric_type0(psi,pot_bak,error_tmp,flag_tmp,cocp)
      call error_metric_type3_mpt(psi,pot_bak,error_tmp,flag_tmp,cocp,impt," psi")
...
! --- alps
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_trG_CF_peos_corot(sou)
      else
...
!!!      call error_metric_type0(alph,pot_bak,error_tmp,flag_tmp,cocp)
      call error_metric_type3_mpt(alph,pot_bak,error_tmp,flag_tmp,cocp,impt,"alph")
...
! --- shift
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_MoC_CF_with_divshift_peos(souvec)
      else
...
!      call reset_metric_CF
      call error_metric_type3_mpt(bvxd,potx_bak,error_tmp,flag_tmp,cocp,impt,"bvxd")
      flag_bvxd  =  max ( flag_bvxd, flag_tmp)
      error_bvxd = dmax1(error_bvxd,error_tmp)
      call error_metric_type3_mpt(bvyd,poty_bak,error_tmp,flag_tmp,cocp,impt,"bvyd")
      flag_bvyd  =  max ( flag_bvyd, flag_tmp)
      error_bvyd = dmax1(error_bvyd,error_tmp)
      call error_metric_type3_mpt(bvzd,potz_bak,error_tmp,flag_tmp,cocp,impt,"bvzd")
...
!
      ...
      if(iter_count<5 .and. iseq<=1 .and. indata_type.eq.'1D') then
        write(6,'(a19)') '-- SKIP HYDRO --   ' 
        error_emd_last = 1.0d0 
        cycle
      end if
! ---
! -- Hydro equations.
      if (error_emd_last.le.eps_coc*0.0001d0 .and. error_metr.gt.eps_coc*2.0d0) then
        write(6,*)     ' ## SKIP HYDRO ##'
      else
        if (impt==1 .or. impt==2) then
          do ihy = 1, hydro_iter
...
!      call interpo_fl2gr(emd, emdg)
      ...
      if(impt==10) then
!        write(6,'(a5,1p,e23.15)')  "rs:  ", rs(ntg/2,0)
        if (mod(iter_count,1)==0) then
          write(char1, '(i5)') iter_count
          char2 = adjustl(char1)
          write(char4, '(i5)') impt
...
!            call IO_output_solution_3D_CF_corot_NS_mpt(jj,'.mid')
...
!
    ...
    if (flag_mid==1) then
      do impt=1,nmpt
        call copy_from_mpatch_all_BNS_CF(impt)
        write(2,'(2i5,1p,5e20.12)')   iter_count, impt, ome, ber, radi
        write(6,'(a3,2i5,1p,5e20.12)') "SSS", iter_count, impt, ome, ber, radi, q_c, emdc
      end do
      write(2,*)  '-----------------------------------------------------------------------'
      write(6,*) '------- flag_mid=1 ------ exit iteration -----'
      close(2)
      exit
    end if

    if ((flag_emd==0).and.(flag_psi==0).and.(flag_alph==0).and. &
    &   (flag_bvxd==0).and.(flag_bvyd==0).and.(flag_bvzd==0))      then
      do impt=1,nmpt
        call copy_from_mpatch_all_BNS_CF(impt)
        write(2,'(2i5,1p,5e20.12)')   iter_count, impt, ome, ber, radi
        write(6,'(a3, 2i5,1p,5e20.12)') "TTT", iter_count, impt, ome, ber, radi, q_c, emdc
      end do
      write(2,*)  '-----------------------------------------------------------------------'
...
end subroutine iter_corot_BNS_CF_mpt
