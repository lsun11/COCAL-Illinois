...
! --- Coordinate grids.
  ...
  do ir = 0, nrgtmp
    read(14,'(1p,6e23.15)') rgtmp(ir)
  end do
  do it = 0, ntgtmp
    read(14,'(1p,6e23.15)') thgtmp(it)
  end do
  do ip = 0, npgtmp
    read(14,'(1p,6e23.15)') phigtmp(ip)
  end do
  close(14)
!
...
!
  do ip = 0, npgtmp
    do it = 0, ntgtmp
      do ir = 0, nrgtmp
        read(12,'(1p,6e23.15)') emdg_tmp(ir,it,ip), omeg_tmp(ir,it,ip)
      end do
    end do
  end do
...
! --- Metric potentials.
  ...
  do ip = 0, npgtmp
    do it = 0, ntgtmp
      do ir = 0, nrgtmp
        read(13,'(1p,6e23.15)')  psi_tmp(ir,it,ip), &
        &                       alph_tmp(ir,it,ip), &
        &                       bvxd_tmp(ir,it,ip), &
        &                       bvyd_tmp(ir,it,ip), &
        &                       bvzd_tmp(ir,it,ip)
  ...
  read(13,'(1p,6e23.15)') ome, ber, radi
  close(13)
!
! --- Interpolation
!
