subroutine error_metric_type2_axisym(pot,pot_bak,error,flag,ctype)
  use phys_constant,  only : long
  use grid_parameter, only : nrg, ntg, npg, eps_coc, &
  &      ntgpolp, ntgpolm, ntgeq, npgxzp, npgxzm, npgyzp, npgyzm
  implicit none
...
!       if (irg == 2 ) cycle
        ...
        &   ipg.eq.npgxzp .or.ipg.eq.npgxzm .or.ipg.eq.npg  .or. &
        &   ipg.eq.npgyzp .or.ipg.eq.npgyzm) cycle
...
end subroutine error_metric_type2_axisym
