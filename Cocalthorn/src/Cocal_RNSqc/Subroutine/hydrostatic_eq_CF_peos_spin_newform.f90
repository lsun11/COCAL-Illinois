  ...
  do ipf = 0, npf
    do itf = 0, ntf
      do irf = 0, nrf
        ...
        ovdfc(1) = bvxdf(irf,itf,ipf) + omefc*vphif(1) - wx
        ovdfc(2) = bvydf(irf,itf,ipf) + omefc*vphif(2) - wy
        ovdfc(3) = bvzdf(irf,itf,ipf) + omefc*vphif(3) - wz
...
!        uih2     = dvep2 + 2.0d0*wdvep + w2
        ...
        hut = (lam + 2.0d0*wdvep)/(alpfc2 - w2)
        if ( hut<0.0d0 ) then
          write(6,*)  "hut negative....exiting"
          stop
        end if

        if ( (hut*lam - dvep2)<0.0d0 ) then
          write(6,*)  "hh imaginary....exiting"
          stop
        end if
        hh = sqrt(hut*lam - dvep2)
