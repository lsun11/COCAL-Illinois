...
!
  ...
  implicit none
  integer :: impt, impt_ex, ico, irr, isp, ipo, npoints, iofile
...
!
  open(850,file='rho_ini0.txt',status='old')
  npoints=0
  do
   read(850,*,iostat=iofile)  xcac
   if (iofile > 0)  then
     write(6,*) "Problem reading file rho_ini0.txt...exiting"
     stop
   else if (iofile < 0) THEN
     write(6,*) "Total number of points is =", npoints
     exit
   else
     npoints = npoints + 1
   end if
  ...
  open(860,file='rho_ini0.txt',status='old')
  write(6,'(21a16)') "# xcac", "psica", "alphca", "bvxdca", "bvydca", "bvzdca", "preca", "rhoca", "epsca", &
    &    "vxu", "vyu", "vzu", "vepxfca", "vepyfca", "vepzfca", "kxx", "kxy", "kxz", "kyy", "kyz", "kzz"

  do ipo=1,npoints
  read(860,*,iostat=iofile)  xcac
  ycac=0.0d0
  zcac=0.0d0
...
!  write(6,'(a6,e20.12)') "vzu  =", vzu
   ...
   write(6,'(21e16.8)') xcac, psica, alphca, bvxdca, bvydca, bvzdca, preca, rhoca, &
      &     epsca, vxu, vyu, vzu, vepxfca, vepyfca, vepzfca, kxx, kxy, kxz, kyy, kyz, kzz
   end do
   close(860)   
