subroutine reset_outer_boundary_RNS_CF(char_mp)
...
! Reset boundary condition for NS
! ###NOT FOT BH!!!  For BH, radi*rg should be replaced by rg
!
  if (char_mp.eq.'psi ') then
    psi(nrg,0:ntg,0:npg) = 1.0d0 + admmass_asymp/(2.0d0*radi*rg(nrg))
  end if
  if (char_mp.eq.'alph') then
    alph(nrg,0:ntg,0:npg)=(1.0d0 - (2.0d0*komarmass_asymp-admmass_asymp) &
    &                    /(2.0d0*radi*rg(nrg)))/psi(nrg,0:ntg,0:npg)
  end if
...
!
end subroutine reset_outer_boundary_RNS_CF
