...
!
  if (cline=="ph") then         ! Circulation on any parallel, radius
    ...
    do ipf = 0, npf
!     hu_i = partial_i(Phi)
      fph(ipf,1) = vepxf(irf,itf,ipf)
      fph(ipf,2) = vepyf(irf,itf,ipf)
      fph(ipf,3) = vepzf(irf,itf,ipf)
    end do
    do ipf = 1, npf
...
!      write(6,'(a3,4i5)') "it=", it,irf,itf,ipf
      ...
      fth(it,1) = vepxf(irf,itf,ipf)
      fth(it,2) = vepyf(irf,itf,ipf)
      fth(it,3) = vepzf(irf,itf,ipf)
    end do 
    do it = 1, 2*ntf
