...
include './Subroutine_TOV/equation_peos_isotr.f90'
...
!
  ...
  integer  :: j,i, ii, iter_compa, itmx, iter_radi, itype, iter_mode, &
           &  ndiv, nstep
...
!
  call read_parameter
!
! -- The following routine converts a pw-polytrope into a table
!  call peos2teos(10000)
!  call stergioulas_rns2cocal
!
  call peos_initialize

!  hini=1.009633100000000d+00
!  hini=1.011965d+00
!  call peos_h2qprho(hini, qini, pre, rho0, ene)
!  write(6,'(a14,1p,6e23.15)') "h,q,p,rho,ene:", hini, qini, pre, rho0, ene
!  write(6,*) "============================================================="
!  call peos_q2hprho(qini,hini,pre,rho0,ene)
!  write(6,'(a14,1p,6e23.15)') "q,h,p,rho,ene:", qini, hini, pre, rho0, ene
!  stop
!
  qini = emdini_gcm1
  call peos_q2hprho(qini, hini, pre, rho0, ene)
...
!      open(8, file='ovlas.dat', status='unknown')
!      write(8,*) "=============== iter_compa, iter_radi", iter_compa,iter_radi
!      close(8)
!      if (iter_radi<=10)   write(6,*) "iter_compa,iter_radi, hini=",iter_compa, iter_radi, hini
      do i = 1, nstep
        ii = i
        xe = x0 + h
...
!        if (radi_error <= 2.0*ermx .and. itype == 0) then
!           if (i>=nstep-1)   write(6,*)  iter_radi, i, (yn_iso(j), j=1,10)
!          call printout_TOV_profile_isotr('aa')
!        end if
!
...
!
!      if (radi_error <= 2.0*ermx .and. itype == 0 .and. iter_compa==5 .and. iter_radi==66)  stop


      if (radi_error <= ermx .and. yn_iso(2)>=1.0d0) then
!        write(6,*) "FFFFFFFFFFFFFFFF", radi_error, yn_iso(2)
        if (itype == 0) exit
        if (itype == 1) then
