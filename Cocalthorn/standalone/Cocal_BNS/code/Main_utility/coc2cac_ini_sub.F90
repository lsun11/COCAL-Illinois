...
!______________________________________________
SUBROUTINE coc2cac_ini_sub(CCTK_ARGUMENTS)
!
  use cctk
  implicit none
  ...
  character(50) :: warnline
  character*400 :: dir_path, option_filename
  character(2)  :: id_type
  integer       :: ierr=0, dir_path_len
!
...
! -- Read ID type
  call CCTK_INFO("COCAL_ID:: Reading parameters...")
  option_filename = trim(dir_path(1:dir_path_len)//'/'//'rnspar_mpt1.dat')
  ierr = read_id_type(option_filename,id_type)

  write(warnline,'(a50)') "COCAL_ID:: problem reading file rnspar_mpt1.dat."
  if (ierr.ne.0) call CCTK_WARN(0,warnline)

  select case (id_type)
    case ("CO")
      call CCTK_INFO("COCAL_ID:: Reading corotating BNS ID")
      call coc2cac_co(CCTK_ARGUMENTS)
    case ("IR")
      call CCTK_INFO("COCAL_ID:: Reading irrotational BNS ID")
      call coc2cac_ir(CCTK_ARGUMENTS)
    case ("SP")
      call CCTK_INFO("COCAL_ID:: Reading spinning BNS ID")
      call coc2cac_sp(CCTK_ARGUMENTS)
  end select
...
END SUBROUTINE coc2cac_ini_sub
!
!
!
!
...
!______________________________________________
SUBROUTINE coc2cac_co(CCTK_ARGUMENTS)
!
...
! -- Read parameters
  call CCTK_INFO("Cocal_ID:: Reading parameters for corotating BNS...")
...
!______________________________________________
SUBROUTINE coc2cac_ir(CCTK_ARGUMENTS)
!
  use grid_parameter_binary_excision
  use phys_constant
  use grid_parameter
...
! -- Read parameters
  call CCTK_INFO("Cocal_ID:: Reading parameters for irrotational BNS...")
...
!    write(6,'(6i5)') nrg, ntg, npg, nrf, ntf, npf
    if (impt==1) then
      ...
      call IO_input_CF_flir_export(dir_path(1:dir_path_len)//"/bnsflu_3D_mpt1.las",emd_p1,vep_p1,ome_p1,ber_p1,radi_p1)
    ...
    if (impt==2) then
      ...
      call IO_input_CF_flir_export(dir_path(1:dir_path_len)//"/bnsflu_3D_mpt2.las",emd_p2,vep_p2,ome_p2,ber_p2,radi_p2)
...
!______________________________________________
SUBROUTINE coc2cac_sp(CCTK_ARGUMENTS)
!
  use grid_parameter_binary_excision
  use phys_constant
  use grid_parameter
...
! -- Read parameters
  call CCTK_INFO("Cocal_ID:: Reading parameters for spinning BNS...")
...
!    write(6,'(6i5)') nrg, ntg, npg, nrf, ntf, npf
    if (impt==1) then
      ...
      call IO_input_CF_flsp_export(dir_path(1:dir_path_len)//"/bnsflu_3D_mpt1.las",emd_p1,vep_p1,wxspf_p1,wyspf_p1, &
    ...
    if (impt==2) then
      ...
      call IO_input_CF_flsp_export(dir_path(1:dir_path_len)//"/bnsflu_3D_mpt2.las",emd_p2,vep_p2,wxspf_p2,wyspf_p2, &
