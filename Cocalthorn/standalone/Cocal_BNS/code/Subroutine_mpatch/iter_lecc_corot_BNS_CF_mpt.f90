subroutine iter_lecc_corot_BNS_CF_mpt(iter_count, iseq, iter_eps)
...
!  use interface_sourceterm_MoC_CF_with_divshift_peos
  ...
  use interface_source_lecc_MoC_CF_gp4th
  use interface_IO_output_1D_general

  use interface_hydrostatic_eq_peos_lecc
...
!    iter_extra = iter_extra + 1
    ...
    write(6,'(a10,i4,a100)') "Iteration:",iter_count, &
    &  "---Errors:-psi------------alph------------bvxd------------bvyd------------bvzd------------emd-------"
!
    do impt = 1, nmpt
      write(6,'(a10,i2,a8)', ADVANCE = "NO")  " Patch # =", impt, ""
      write(6,*)  " "
...
!        
        ...
        write(6,'(a16,1p,2e20.12)') "(delome,delvel)=", delome, delvel
        write(6,'(a16,1p,2e20.12)') "(   ome,  radi)=", ome, radi
      else
        cocp = 'bh'
...
! --- psi
      ...
      write(6,'(a21,1p,4e20.12)') "(velx,ome,radi,emdc)=", velx, ome, radi, emdc
...
!        if(impt==1 .and. (iter_count<10 .or. error_psi_last<=1.0d-12))  then
!        if(impt==1 )  then
!          write(char1, '(i5)') iter_count
!          char2 = adjustl(char1)
!          write(char4, '(i5)') impt
!          char5 = adjustl(char4)
!          char3 = 'sou_iter' // trim(char2) // '_mpt' // trim(char5) // '.txt'
!          call IO_output_1D_general(char3,'g','m',sou,-1,ntg/2,0)
!        end if
      else
...
!          write(char4, '(i5)') impt
!          char5 = adjustl(char4)
!          char3 = 'sou_surf_iter' // trim(char2) // '_mpt' // trim(char5) // '.txt'
!          open(12,file=char3,status='unknown')  
!          ipg=0
!          do itg=0,ntg
!            write(12,'(i5,1p,5e20.12)')  itg, thg(itg), sou_exsurf(itg,ipg), dsou_exsurf(itg,ipg), &
!             &                                         sou_outsurf(itg,ipg),dsou_outsurf(itg,ipg)
!          end do
!          close(12)
!        end if
...
!          write(char4, '(i5)') impt
!          char5 = adjustl(char4)
!          char3 = 'psi_iter' // trim(char2) // '_mpt' // trim(char5) // '.txt'
!          open(12,file=char3,status='unknown')
!          itg=ntg/2;  ipg=0
!          do irg=0,nrg
!            write(12,'(i5,1p,3e20.12)')  irg, rg(irg), psi(irg,itg,ipg), pot(irg,itg,ipg)
!          end do
!          close(12)
!        end if
 
!        write(6,'(a20,1p,3e20.12)') "*psi ",psi(nrf,ntg/2,0),psi(0,0,0),psi(nrf,ntg/2,npg/2)

      pot_bak(0:nrg,0:ntg,0:npg) = psi(0:nrg,0:ntg,0:npg)
      call update_grfield(pot,psi,conv_gra)

!        write(6,'(a20,1p,3e20.12)') "*psi_new ",psi(nrf,ntg/2,0),psi(0,0,0),psi(nrf,ntg/2,npg/2)

      if ((impt==1 .or. impt==2) .and. (iter_count>5 .or. iseq>1 .or. indata_type.eq.'3D')) &
          &     call update_parameter_BNS(conv_den)

!      write(6,'(a16,1p,3e20.12)') "(   ome,  radi)=", ome, radi, emdc
...
!        call sourceterm_MoC_CF_with_divshift_peos(souvec)
        call source_lecc_MoC_CF_gp4th(souvec)
      ...
      if (error_emd_last.le.eps*0.0001d0 .and. error_metr.gt.eps*2.0d0) then
        ...
        if (impt==1 .or. impt==2) then
          do ihy = 1, hydro_iter
            call interpo_gr2fl_metric_CF
            call rotation_law
            call hydrostatic_eq_peos_lecc(potf)
...
!      error_emd_last = error_emd
!
      if(impt==5 .and. (iter_count<10 .or. error_psi_last<=1.0d-12))  then
!      if(impt==1 ) then
...
end subroutine iter_lecc_corot_BNS_CF_mpt
