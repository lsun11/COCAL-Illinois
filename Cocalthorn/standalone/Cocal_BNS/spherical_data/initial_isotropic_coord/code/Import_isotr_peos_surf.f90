...
!              Main Program
!______________________________________________
PROGRAM Import_isotr_peos_surf
!
  ...
  use grid_parameter_1D, only : iter_max, rgmid, nrgin, eps, emdc, nrg, nrf, nrg_1, r_surf
...
!
  ...
  call coordinate_patch_kit_grav_1D_surf(3)
!
  nlines = 0
...
!    read(3,'(5es15.7)') rg_iso(i), psi(i), alph(i), emdg(i), h_iso(i)
    read(3, '(17(es15.7))') xecgs, m_iso(i), h_iso(i), prm_iso(i), pm_iso(i), &
    &     psi(i), dummy, alph(i), rs_iso(i), rho0_iso(i), rhocgs,   &
    &     pre_iso(i), precgs, ene_iso(i), epsiloncgs, xe, rg_iso(i)
    ber = ber + alph(i)*h_iso(i)
    emdg(i) = pre_iso(i)/rho0_iso(i)
  end do
  ber = ber/nlines
  close(3)

! rg_iso comes out of TOV_isotr and is equal to xe/radi, where xe is the isotropic radial 
! coordinate and radi the final radius (isotropic) of star. Thus it is normalized  to 1.0.
  rg_iso(0:nnrg) = r_surf*rg_iso(0:nnrg)

  open(4,file='ovphy_plot.dat',status='unknown')
  read(4 , '(13es14.6)') compa, emdc, rho0, rhocgs, pre, ene, &
  &       xi, xicgs, restmass_sph, propermass_sph, adm, gravmass_sph, dummy
  close(4)
!  radi = xi
  radi = xi/r_surf
  rs_normal = 2.0d0*r_surf/(1.0d0 + (1.0d0-2.0d0*compa)**0.5 - compa)
  m_normal = compa*rs_normal

  call interpg_ini_iso(nlines-1,rg_iso,psi)
  call interpg_ini_iso(nlines-1,rg_iso,alph)
  call interpg_ini_iso(nlines-1,rg_iso,emdg)
!
...
!
  ...
  write(4,'(5i5)') nrf
  do irg = 0, nrf
    write(4,'(6es20.12)') rg(irg), emdg(irg)
  end do
...
!  call printout_plot
!
END PROGRAM Import_isotr_peos_surf
!______________________________________________
!
