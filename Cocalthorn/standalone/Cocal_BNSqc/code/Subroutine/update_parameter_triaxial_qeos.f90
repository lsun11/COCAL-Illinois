...
! "ov2in" is f_{ab} \omega^a \omega^b.
  ...
  real(long)     ::    facfac, numero, error, hc, hs, pre, rho, ene
  real(long)     ::    dummy
  integer        ::    nin
!
! ################################################################       
...
! termin is 1-(\phi^4/\alpha^2)^((R_0/R_i)^2) \omega^2            
    dtermdoin = - pain**(2.d0*radi**2)*2.0d0*ovin*vphixin
...
!
    sgg(1) = -(radi**2*ain  + 0.5d0*dlog(termin)  - dlog(ber) + loghs)
    sgg(2) = -(radi**2*aout + 0.5d0*dlog(termout) - dlog(ber) + loghs)
