...
include '../../code/EOS/Subroutine/mit_initialize.f90'
include '../../code/EOS/Subroutine/mit_ene2prho.f90'
include './Module_TOV/def_TOV_quantities.f90'
include './Subroutine_TOV/rk_mit.f90'
include './Subroutine_TOV/rkstep_mit.f90'
include './Subroutine_TOV/equation_mit_isotr.f90'
include './Subroutine_TOV/printout_TOV_profile_isotr_mit.f90'
include './Subroutine_TOV/printout_TOV_quantities_isotr_mit.f90'
...
!
  ...
  use def_qeos_parameter, only : rhoini_gcm1, aq, eneini_gcm1, enesurf_gcm1
...
!
  ...
  real(8)  :: q1cgs, h1cgs
  real(8)  :: pre1cgs, rho1cgs, ene1cgs, dpdrho1, eneinib
!
  ermx      = 1.0d-10
  ermx_MorC = 1.0d-9
  itmx = 1000
!
  open (2, file='ovpar_mit.dat', status='old')
...
!    
      x0 = 0.0d+0 ; xn = radi
      y0_iso(1) = 0.0d+0 ; y0_iso(2) = eneini ; y0_iso(3) = 0.0d+0
...
!          call peos_h2qprho(hini, q1, pre1, rho1, ene1)
          ene1=eneini
          call mit_ene2prho(ene1, pre1, rho1)
          rho1cgs = rho1*(c**6/((g**3)*solmas**2))
          pre1cgs = pre1*c**8/(g**3*solmas**2)
          ene1cgs = ene1*c**8/(g**3*solmas**2)
          write(12, '(17(es15.7))') 0.0d0, 0.0d0, ene1, 0.0d0, 0.0d0, &
...
!          write(12,'(5es15.7)') x0, y0_iso(5), y0_iso(8), q1, hini
      ...
      yn_iso(1) = 0.0d+0 ; yn_iso(2) = eneini ; yn_iso(3) = 0.0d+0 
...
!
      do i = 1, nstep
        ii = i
        xe = x0 + h
        call rk_mit(neq_iso, equation, x0, xe, ndiv, y0_iso, yn_iso, work_iso, aq)
        if (yn_iso(2) <= enesurf_gcm1) exit
!
        if ((radi_error <= ermx .and. itype == 1) .or. (radi_error <= ermx .and. erer < ermx_MorC)) then
          file_ctl = 'nm'
          if (i.eq.1)     file_ctl = 'op'
          if (i.eq.nstep) file_ctl = 'cl'
          call printout_TOV_profile_isotr_mit(file_ctl)
        end if
...
!      if (radi_error <= ermx .and. yn_iso(2)>1.0d0) then
      if (radi_error <= ermx .and. yn_iso(2)>enesurf_gcm1) then
        if (itype == 0) exit
        if (itype == 1) then
          call printout_TOV_quantities_isotr_mit
...
!
    if (iter_compa == 1) then
      ...
      dqdq = dabs(dqdq)
      erer = 1.0d0
      write(6, '(1i5, es12.4, 3es16.8)') iter_compa, erer, eneinib, Q_current
      cycle
    end if
!
    if (erer < ermx_MorC) then
...
!
    dqdq = - (Q_current-Q_target)/(Q_current-Q_current_bak)*(eneini-eneinib)
    Q_current_bak = Q_current
    eneinib= eneini
    eneini = eneini + dqdq
!
