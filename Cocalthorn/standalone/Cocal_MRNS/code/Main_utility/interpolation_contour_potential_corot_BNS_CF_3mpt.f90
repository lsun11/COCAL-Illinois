...
!______________________________________________
PROGRAM interpolation_contour_potential_corot_BNS_CF_3mpt
!
...
!
  do impt = 1, nmpt
    ...
    call IO_input_converged_solution_3D_CF_NS_mpt(impt)
...
!
  ...
  do impt = 1, nmpt
    ...
    call IO_output_plot_xyz_BNS_CF_mpt(impt)
    if (impt==1 .or. impt==2)  then
      call interpo_gr2fl_metric_CF

!@      call interpolation_cartesian_BNS_mpt(impt)
!@      write(6,*) "Patch:", impt, ", writing contour files..."
!@      call IO_output_cartesian_contour_BNS_mpt(impt)
...
!cal      call calc_4velocity_ut_corot
!
!cal      call calc_rest_mass_peos      ! integral over star
!cal      call calc_mass_peos           ! integral over star only for Komar mass
!cal      call calc_proper_mass_peos    ! integral over star
!cal      call calc_ang_mom_peos        ! integral over star
...
!cal      call calc_circ_line_peos
!cal      call calc_circ_surf_peos
...
!cal      call excurve_CF(cocp)         !3rd order midpoint from ir0=0,...
!cal      call excurve_CF_gridpoint_bhex    !   4th order from ir0=0,...

!cal      write(6,*) '----------------------------------------- dis, radi:', dis, radi
...
!
END PROGRAM interpolation_contour_potential_corot_BNS_CF_3mpt
