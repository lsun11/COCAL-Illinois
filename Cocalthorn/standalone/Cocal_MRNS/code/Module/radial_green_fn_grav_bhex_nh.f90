...
module radial_green_fn_grav_bhex_nh
  use phys_constant, only : long
  use grid_parameter, only : nrg, nlg, rgin, rgout
  use coordinate_grav_r, only : rg, rginv, hrg, hrginv
  use make_array_3d
  implicit none
  real(long), pointer  ::  hgfn_nh(:,:,:), gfnsf_nh(:,:,:)
contains
subroutine allocate_hgfn_bhex_nh
  implicit none
  call alloc_array3d(hgfn_nh,1,nrg,0,nlg,0,nrg)
  call alloc_array3d(gfnsf_nh,0,nlg,0,nrg,1,4)
end subroutine allocate_hgfn_bhex_nh
! ----------------------------------------------
! Subroutine
! hgfn(irr,il,ir) --> for r' < r
!                     for r  < r'
! _nh --> no boundary and BH neumann
!         with homogeneous solution
! N.B.  rg(0) = rgin should not equal to zero
! ----------------------------------------------
subroutine calc_hgfn_bhex_nh
  implicit none
  integer     ::  ir, irr, il
  ...
  do ir  = 0, nrg
    do irr = 1, nrg
      if (hrg(irr).lt.rg(ir)) then
        do il  = 0, nlg
          hgfn_nh(irr,il,ir) = hrg(irr)**il/rg(ir)**(il+1)
        end do
      end if
      if (hrg(irr).ge.rg(ir)) then
        do il  = 0, nlg
! -- 1.
          hgfn_nh(irr,il,ir) = rg(ir)**il/hrg(irr)**(il+1)
        end do
      end if
    end do
  end do
...
!
  do ir = 0, nrg
!
    il = 0  ! only l=0 mode neumann b.c. for the outer surface
    gfnsf_nh(il,ir,1) = - dble(2*il+1)/dble(il+1) &
    &                 *bhrad*(bhrad*rginv(ir))**(il+1)
    gfnsf_nh(il,ir,2) = 0.0d0
    gfnsf_nh(il,ir,3) = 0.0d0
    gfnsf_nh(il,ir,4) = - dble(il+1)
!
    do il = 1, nlg
!
      gfnsf_nh(il,ir,1) = - dble(2*il+1)/dble(il+1) &
      ...
      gfnsf_nh(il,ir,2) = 0.0d0
      gfnsf_nh(il,ir,3) = 0.0d0
      gfnsf_nh(il,ir,4) = 0.0d0
!
    end do
  end do
!
end subroutine calc_hgfn_bhex_nh
end module radial_green_fn_grav_bhex_nh
