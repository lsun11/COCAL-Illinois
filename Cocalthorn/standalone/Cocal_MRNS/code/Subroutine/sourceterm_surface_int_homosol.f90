subroutine sourceterm_surface_int_homosol(fnc,irg_surf,sou_surf,dsou_surf)
  use phys_constant, only : long
  use grid_parameter, only : ntg, npg
  use make_array_2d
  use interface_interpo_linear_type0_2Dsurf
  use interface_grdr_gridpoint_type0_nosym
  use interface_grdr_gridpoint_type0_3rd_nosym
!
  implicit none
...
!!!      call grdr_gridpoint_type0_3rd_nosym(fnc,deriv,irg_surf,itg,ipg)
      call grdr_gridpoint_type0_nosym(fnc,deriv,irg_surf,itg,ipg)
      dfnc_surf(itg,ipg) = deriv
    end do
  end do
! Modify the value of the source for 
! the surface terms sou_surf, dsou_surf
! such as  pot_0 to pot_0 - pot_INT
  do ipg = 1, npg
    do itg = 1, ntg
      call interpo_linear_type0_2Dsurf(val,fnc_surf,itg,ipg)
      sou_surf(itg,ipg) = sou_surf(itg,ipg) - val
      call interpo_linear_type0_2Dsurf(val,dfnc_surf,itg,ipg)
      dsou_surf(itg,ipg) = dsou_surf(itg,ipg) - val
    end do
  end do
  deallocate(fnc_surf)
