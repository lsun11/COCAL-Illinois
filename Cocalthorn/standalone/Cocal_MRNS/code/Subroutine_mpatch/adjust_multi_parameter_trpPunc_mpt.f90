subroutine adjust_multi_parameter_trpPunc_mpt(flag_param,istep_niq)
  use phys_constant, only  : long
  use grid_parameter, only : mass_eps, eps
  use interface_minv
  use interface_adjust_copy_trpPunc_from_mpt
  use interface_adjust_copy_trpPunc_to_mpt
...
! Set next parameters
  if (istep_niq.ge.0.and.istep_niq.lt.niq) then
    ...
    call adjust_copy_trpPunc_to_mpt(niq,msec_x_der)
    istep_niq = istep_niq + 1
    flag_param = 1
    return
  end if
...
!
end subroutine adjust_multi_parameter_trpPunc_mpt
