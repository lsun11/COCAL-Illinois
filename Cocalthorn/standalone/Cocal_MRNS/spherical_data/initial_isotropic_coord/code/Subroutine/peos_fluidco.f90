...
!
  ...
  real(8) :: alpfc, emdfc, hhfc, pre, rho, ene, hh
...
!    emd(ir) = 1.0d0/(pinx+1.0d0) * (ber/alphf(ir) - 1.0d0)
    ...
    if (hh <= 1.0d0) then
      ...
      call peos_h2qprho(hh, emd(ir), pre, rho, ene)
    end if
  end do
!
  emd(0) = emdc
...
!
  ...
  write(6,"(a22,es12.4,',  value =',es12.4,2x,3i4)") &
  &       '  == emd   ==, error =', emxemd, emd(irerr), irerr
!
! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
! --  Improving parameters and alpha and psi.  
!
  call peos_paimproco_frg(ber,radi,convf,iter,fmax0,1)
!
! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
!mark
!
