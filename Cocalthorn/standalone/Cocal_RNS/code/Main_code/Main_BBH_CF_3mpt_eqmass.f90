...
!              Main Program
!______________________________________________
PROGRAM Main_BBH_CF_3mpt_eqmass
!
...
if(iseq.eq.1) then
  do impt = 1, nmpt
    ...
    if (indata_type.eq.'IN') call initial_metric_CF_BH_mpt(impt)
    if (indata_type.eq.'3D') call IO_input_initial_3D_CF_BH_mpt(impt)
    call copy_def_metric_to_mpt(impt)
  end do
end if
!
  call iteration_BBH_CF_3mpt_eqmass(iter_count)
...
!
  ...
  call printout_physq_BBH_mpt(iseq)
!
...
!
END PROGRAM Main_BBH_CF_3mpt_eqmass
