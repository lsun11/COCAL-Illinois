...
! Coordinates wrt the CM
      ...
      dBdx = 0.25d0*(dBfundx(0,itg,ipg)   + dBfundx(0,itg-1,ipg)  &
                   + dBfundx(0,itg,ipg-1) + dBfundx(0,itg-1,ipg-1) )

      bf = 0.25d0*(Bfun(0,itg,ipg)   + Bfun(0,itg-1,ipg)  &
                 + Bfun(0,itg,ipg-1) + Bfun(0,itg-1,ipg-1) )

!      sou_surf(itg,ipg) = -0.08d0*(-rcm*sin(tcm)*sin(pcm)) + 0.25d0*dBdx/bf
!      sou_surf(itg,ipg) = -0.08d0*(-rcm*sin(tcm)*sin(pcm)) + 0.25d0*dBdx
      sou_surf(itg,ipg) = -0.08d0*(-rcm*sin(tcm)*sin(pcm))
!      sou_surf(itg,ipg) = -0.5d0*0.08d0*(-rcm*sin(tcm)*sin(pcm))
    end do
  end do
!
