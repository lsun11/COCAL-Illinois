subroutine calc_ToverW_WL
  use phys_constant, only  : long, pi
  use grid_parameter, only : nrg, ntg, npg, nrf, ntf, npf
  use def_matter, only : omef, omeg
...
!
  ...
  soug(0:nrg,0:ntg,0:npg) = omeg(0:nrg,0:ntg,0:npg)*soug(0:nrg,0:ntg,0:npg)
  call vol_int_fluid(souf,volf)
  call vol_int_grav(soug,volg)
!  T_kinene_omeJ = 0.5d0*radi**3*volf
  fac8pi = 0.125d0/pi
  T_kinene_omeJ = 0.5d0*(radi**3*volf + fac8pi*radi*volg) 
