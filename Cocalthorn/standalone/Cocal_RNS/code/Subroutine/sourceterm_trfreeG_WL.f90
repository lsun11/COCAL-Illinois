subroutine sourceterm_trfreeG_WL(souten)
  ...
  use def_ricci_tensor, only : rabnl, rabDF
  ...
  use def_formulation, only : swlp, swls, iswl
...
!
  ...
  real(8) :: rlieaij(3,3), rnlab(3,3), rpsiab(3,3), &
  &          aacabc(3,3),   sab(3,3), ovd(3), ovu(3), rpddal(3,3)
...
!
  ...
  &          ogdphdph, ovpadpsi, ovxgc, ovygc, ovzgc, &
  ...
  &          hhyxd, hhyyd, hhyzd, hhzxd, hhzyd, hhzzd, &
  &          ene, grad(1:3), traceK
  integer :: ipg, irg, itg, iph, ii, ia, ib, ic
!
...
!!        end do
        ...
        dbv(1:3,1:3) = gradb(irg,itg,ipg,1:3,1:3)
        daij(1:6,1:3) = gradk(irg,itg,ipg,1:6,1:3)
        traceK = trk(irg,itg,ipg)
...
!        call dadbscalar_type0(alps2,dabalps2,irg,itg,ipg)
        call dadbscalar_type3(alps2,dabalps2,irg,itg,ipg)
!
...
! ### rotating shift
!
        ovpadpsi=ps43oal*(ovu(1)*dapsi(1)+ovu(2)*dapsi(2)+ovu(3)*dapsi(3))
...
!
          ...
          rlplbg(3,1)=rlplbg(1,3)
          rlplbg(3,2)=rlplbg(2,3)
          rlplbg(3,3)=rlplbg(3,3)
!
...
!
          rlieaij(ia,ib) = &
          &     ovu(1)*daij(ic,1) + ovu(2)*daij(ic,2) + ovu(3)*daij(ic,3) &
...
!          end if
!
! --- R_ab(DF term) + R_ab(nonlinear) 
!
          rnlab(ia,ib) = rabDF(irg,itg,ipg,ic) + rabnl(irg,itg,ipg,ic)
!
...
!
          souten(irg,itg,ipg,ic) = &
             &      2.0d0*( tfheli + tfliep4aij &
             &            + tfrnlab + tfrpddal  &
             &            + san*psigc4*traceK*tfaij - 2.0d0*psigc4*tfaa &
...
!!write(6,'(1p,3e16.8)') tfheli, + tfliep4aij
!!write(6,'(1p,3e16.8)') tfrnlab, + tfrpddal
!!write(6,'(1p,3e16.8)') + san*psigc4*traceK*tfaij, - 2.0d0*psigc4*tfaa
!!write(6,'(1p,3e16.8)') - roku*gamdhdh, + roku*ogdphdph
!!write(6,'(1p,3e16.8)') traij, trlieaij, tflieaij
!!write(6,'(1p,3e16.8)') daij(1,1), daij(1,2), daij(1,3)
!!end if
!
!testtest
!      include 'GR_IRDR_souten_output_rpal.f'
!testtest
!
        end do
      end do
