...
! In this case the iteration starts from TOV initial data (or 3D) computes the first spinning solution       !
! and then increases emdc to move to the next spinning solution.                                             !
! When computing parallel spinning solutions starting from different TOV use numseq=1 and the script         !
  ...
  do impt = 1, nmpt
    ...
    if (indata_type.eq.'3D') then
      if(NS_shape.eq.'SP')  then
        write(6,*) "########################### Reading 3D Spinning Initial Data ######################################"
        call IO_input_initial_3D_CF_spin_NS_mpt(impt)
        if (impt==1 .or. impt==2)    then
          emdc = emd(0,0,0)
        ...
        write(6,*) "**************** indata_type=3D and NS_shape is neither IR nor CO or SP....exiting"
        stop
      end if
    else                                     !  1D initial data 
      if (impt==1 .or. impt==2)  then
        call  IO_input_initial_1D_CF_NS_mpt(impt) 
        emdc = emd(0,0,0)
        call initial_velocity_potential_NS_mpt(impt)
        if(NS_shape.eq.'SP')   call IO_input_initial_vrot_NS_mpt(impt) 
      else
        emdc = 0.0d0
      end if
    end if
...
!
  ...
  if ( sw_mass_iter=='y' ) then
    call calc_spin_BNS_CF_mpt(total_iteration)
    ...
    do impt = 1, nmpt
      ...
      call copy_def_metric_and_matter_from_mpt(impt)
      call copy_def_matter_spin_from_mpt(impt)
      if (outdata_type.eq.'3D')  call IO_output_solution_3D_CF_spin_NS_mpt(impt,'.las')
...
!     Usually this is the case when the initial data are 3D.
      iter_eps = 1.0d-02
      call iter_spin_BNS_CF_mpt(iter_count,iseq, iter_eps)
      call calc_physical_quantities_spin_BNS_CF_mpt
      char3 = 'main_bnsphys_all_mpt.txt'
      call printout_physq_BNS_all_mpt(char3)
      call write_last_physq_BNS_mpt
...
!
END PROGRAM Main_spin_BNS_CF_mpt
