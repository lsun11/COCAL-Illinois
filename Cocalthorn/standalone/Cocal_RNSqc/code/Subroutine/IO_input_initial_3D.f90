subroutine IO_input_initial_3D
  ...
  use make_array_3d
  implicit none
  real(long), pointer :: emd_tmp(:,:,:),  omef_tmp(:,:,:), rs_tmp(:,:)
...
!
  ...
  call alloc_array3d( emd_tmp,0,nrfmax,0,ntfmax,0,npfmax)
...
!
  do ip = 0, npftmp
    do it = 0, ntftmp
      do ir = 0, nrftmp
        read(12,'(1p,6e20.12)') emd_tmp(ir,it,ip), rs_tmp(it,ip), &
        &                      omef_tmp(ir,it,ip)
      end do
    end do
  end do
...
!
  if (nrf.ne.nrftmp.or.ntf.ne.ntftmp.or.npf.ne.npftmp) then
    call interpo_3D_initial_4th( emd_tmp,'sfco')
...
!
   emd(0:nrf,0:ntf,0:npf) =  emd_tmp(0:nrf,0:ntf,0:npf)
...
!
  ...
  deallocate(bvzd_tmp)
!
end subroutine IO_input_initial_3D
