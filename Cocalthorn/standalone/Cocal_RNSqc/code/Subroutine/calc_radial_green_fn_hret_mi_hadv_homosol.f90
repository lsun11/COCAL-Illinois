subroutine calc_radial_green_fn_hret_mi_hadv_homosol(omega,char_dn)
...
!  sbsjy_hrmiha_ho(l,m,ir) :=  y_l(m omega r)/W
!  sbsjyp_hrmiha_ho(l,m,ir) =  y'_l(m omega r)/W'
!  W  = sqrt(j_l(m omega R)^2  + n_l(m omega R)^2)
!  W' = sqrt(j'_l(m omega R)^2 + n'_l(m omega R)^2)
!
! For m = 0, use 
!  sbsjy_hrmiha(l,0,ir)  = r^l/R^(l+1)
!  sbsjyp_hrmiha(l,0,ir) = - (l+1) r^l/R^(l+2)
!
  sbsjy_hrmiha_ho  = 0.0d0
  sbsjyp_hrmiha_ho = 0.0d0
  do il = 0, nlg
    do im = 0, il
      do ir = 0, nrg
        if(im.eq.0)then
...
!            sbsjy_hrmiha_ho(il,im,ir)= -l2p1dlp1*rg(0)**(il+2)/rg(ir)**(il+1)
            sbsjy_hrmiha_ho(il,im,ir) = 0.0d0  ! 1/2(Gout - Gin)
          end if  
          if (char_dn.eq.'dh') then
!            sbsjyp_hrmiha_ho(il,im,ir)= -l2p1*(rg(0)/rg(ir))**(il+1)
            sbsjyp_hrmiha_ho(il,im,ir) = 0.0d0  ! 1/2(Gout - Gin)
          end if  
        else
          ...
          sby = sj  ! 1/2(Gout - Gin)
          if (char_dn.eq.'nh') then 
            sbsjy_hrmiha_ho(il,im,ir)  = l2p1*sby/Wp
          end if
          if (char_dn.eq.'dh') then
!            sbsjyp_hrmiha_ho(il,im,ir) = - l2p1*sby/W   ! original?
            sbsjyp_hrmiha_ho(il,im,ir) = l2p1*sby/W
          end if
        endif
      enddo !ir
    enddo ! im 
  enddo ! il
!
end subroutine calc_radial_green_fn_hret_mi_hadv_homosol
