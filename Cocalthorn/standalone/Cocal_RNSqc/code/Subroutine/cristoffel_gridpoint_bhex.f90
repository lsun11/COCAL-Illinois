subroutine cristoffel_gridpoint_bhex
  ...
  &          gmyxu, gmzxu, gmzyu, dgdx,dgdy,dgdz, gg
  integer :: ipg, itg, irg
!
...
!
        ...
        gmzyu = gmyzu
!
!        call grgrad_4th_gridpoint_bhex(detg_grid_ks,dgdx,dgdy,dgdz,irg,itg,ipg)
...
!
        gmcrix_grid(irg,itg,ipg) = -dihixu_grid(irg,itg,ipg)  
        gmcriy_grid(irg,itg,ipg) = -dihiyu_grid(irg,itg,ipg)  
        gmcriz_grid(irg,itg,ipg) = -dihizu_grid(irg,itg,ipg)  

!        if (chgra == 'k') then
!          gg = 2.0d0*detg_grid_ks(irg,itg,ipg)
!
!          gmcrix_grid(irg,itg,ipg) = -Fxu_grid_ks(irg,itg,ipg)    &
!                    &                -(gmxxu*dgdx + gmxyu*dgdy + gmxzu*dgdz)/gg
!
!          gmcriy_grid(irg,itg,ipg) = -Fyu_grid_ks(irg,itg,ipg)    &
!                    &                -(gmyxu*dgdx + gmyyu*dgdy + gmyzu*dgdz)/gg
!
!          gmcriz_grid(irg,itg,ipg) = -Fzu_grid_ks(irg,itg,ipg)    &
!                    &                -(gmzxu*dgdx + gmzyu*dgdy + gmzzu*dgdz)/gg
