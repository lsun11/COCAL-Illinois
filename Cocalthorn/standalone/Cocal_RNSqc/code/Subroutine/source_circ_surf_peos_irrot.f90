subroutine source_circ_surf_peos_irrot(cline, ia, ib, souf)
...
!    call alloc_array2d(fz, 0, ia, 0, npf)
    ...
    do irf = 0, ia
      do ipf = 0, npf
!       hu_i = partial_i(Phi)
        fx(irf,ipf) = vepxf(irf,itf,ipf)
        fy(irf,ipf) = vepyf(irf,itf,ipf)
!        fz(irf,ipf) = hh*ui(3)
      end do
    end do
    do irf = 1, ia
  ...
  if (cline=="th") then   ! Circulation on any meridian plane, radius
    ...
    do irf = 0, ia
      ...
      do itf = 0, ntf      !  Half of the disk 
        fx(irf,itf) = vepxf(irf,itf,ipf)
        fy(irf,itf) = vepyf(irf,itf,ipf)
        fz(irf,itf) = vepzf(irf,itf,ipf)
      end do
      ...
      do it = ntf, 2*ntf
        ...
        gx(irf,itf) = vepxf(irf,itf,ipf)
        gy(irf,itf) = vepyf(irf,itf,ipf)
        gz(irf,itf) = vepzf(irf,itf,ipf)
      end do
    end do
...
end subroutine source_circ_surf_peos_irrot
