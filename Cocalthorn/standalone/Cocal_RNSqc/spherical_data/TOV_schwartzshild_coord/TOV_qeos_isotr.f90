...
include '../../code/EOS/Function/quark_rho2h_dot.f90'
...
!
  external equation
  real(8), external :: quark_rho2p, quark_rho2ene, quark_rho2h, quark_rho2h_dot
...
!    
      x0 = 0.0d+0 ; xn = radi
      y0_iso(1) = 0.0d+0 ; y0_iso(2) = rhoini ; y0_iso(3) = 0.0d+0
...
!          call peos_h2qprho(hini, q1, pre1, rho1, ene1)
          ...
          write(12, '(18(es15.7))') 0.0d0, 0.0d0, rho1, 0.0d0, 0.0d0, &
          &     y0_iso(5), 0.0d0, y0_iso(8), 0.0d0, hini, rho1cgs, pre1, &
          &                        pre1cgs, ene1, ene1cgs, 0.0d0, 0.0d0, c_sc
...
!          write(12,'(5es15.7)') x0, y0_iso(5), y0_iso(8), q1, hini
      ...
      yn_iso(1) = 0.0d+0 ; yn_iso(2) = rhoini ; yn_iso(3) = 0.0d+0 
...
!
      do i = 1, nstep
        ...
        if (yn_iso(2) <= rhosurf_gcm1) exit
!
        if ((radi_error <= ermx .and. itype == 1) .or. (radi_error <= ermx .and. erer < ermx_MorC)) then
          file_ctl = 'nm'
          if (i.eq.nstep/400)     file_ctl = 'op'
          if (i.eq.nstep) file_ctl = 'cl'
          if (mod(i,nstep/400).eq.0) call printout_TOV_profile_isotr_quark(file_ctl)
        end if
...
!      if (radi_error <= ermx .and. yn_iso(2)>1.0d0) then
      if (radi_error <= ermx .and. yn_iso(2)>rhosurf_gcm1) then
        if (itype == 0) exit
        if (itype == 1) then
          call printout_TOV_quantities_isotr_quark
...
!
    rho_top = 2.0d0*rhoini
    rho_bot = rhosurf_gcm1
...
!
    if (iter_compa == 1) then
      ...
      hinib  = hini
      hini = hini + dqdq
      call quark_h2rho(quark_rho2h,quark_rho2h_dot,rho_bot,rho_top,hini,rhoini)
      dqdq = dabs(dqdq)
      erer = 1.0d0
