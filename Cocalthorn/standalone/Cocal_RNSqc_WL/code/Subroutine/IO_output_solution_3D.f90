subroutine IO_output_solution_3D
...
! --- Matter
  ...
  do ip = 0, npf
    do it = 0, ntf
      do ir = 0, nrf
        write(12,'(1p,6e20.12)') emd(ir,it,ip), rs(it,ip), omef(ir,it,ip)
      end do
    end do
  end do
  ...
  open(15,file='rnssur_3D.las',status='unknown')
...
! --- Coordinate grids.
  ...
  close(14)
!
end subroutine IO_output_solution_3D
