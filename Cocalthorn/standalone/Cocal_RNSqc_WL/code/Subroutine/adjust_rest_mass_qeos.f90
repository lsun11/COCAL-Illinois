subroutine adjust_rest_mass_qeos(flag_restmass,count_adj)
  use grid_parameter, only : mass_eps
  use def_matter_parameter, only : rhoc_qs
  use def_quantities
  implicit none
...
!
  if(flag_restmass.eq.0) then 
    epsmax_par = 1.0d0
    if (count_adj.ne.0) epsmax_par = dabs((rhoc_qs - par_val)/rhoc_qs)
    par_val = rhoc_qs
    fnc_val = restmass
!    fnc_val = fixeddlm 
...
!test 05AUG2012    emdc = par_val + sgg/gg*fac0
    rhoc_qs = rhoc_qs + sgg/gg*fac0
    epsmax_fnc = dmax1(epsmax_fnc,dabs(par_val - rhoc_qs)/dabs(par_val))
...
!
    ...
    write(6,'(1a30,1p,2e12.4)')' -- rhoc   OLD -> NEW     --  ',par_val,rhoc_qs
!
...
end subroutine adjust_rest_mass_qeos
