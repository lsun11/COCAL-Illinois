subroutine iter_lecc_irrot_BNS_CF_mpt(iter_count, iseq, iter_eps)
...
!
  ...
  use interface_sourceterm_lecc_HaC_peos_irrot
  use interface_sourceterm_HaC_CF
  use interface_sourceterm_lecc_trG_peos_irrot
  use interface_sourceterm_trG_CF
  use interface_sourceterm_lecc_MoC_CF_with_divshift_peos_irrot
...
!  use interface_violation_gridpoint_HaC_CF_peos_irrot
  use interface_hydrostatic_eq_CF_peos_lecc_irrot
  use interface_calc_gradvep
  use interface_hydro_irbns_vep_CF_peos_lecc
...
! --- psi
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_lecc_HaC_peos_irrot(sou)
      else
...
!        call outer_boundary_d_BBH_CF(sou_outsurf,'psi ')
      ...
      if ((impt==1 .or. impt==2) .and. (iter_count>5 .or. iseq>1 .or. indata_type.eq.'3D')) &
        &          call update_parameter_BNS(conv_gra)
...
! --- alps
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_lecc_trG_peos_irrot(sou)
      else
...
! --- shift
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_lecc_MoC_CF_with_divshift_peos_irrot(souvec)
      else
      ...
      if (error_emd_last.le.eps*0.0001d0 .and. error_vep_last.le.eps*0.0001d0 ) then
        ...
        if (impt==1 .or. impt==2) then
          do ihy = 1, hydro_iter
            call interpo_gr2fl_metric_CF
            call rotation_law
            call hydrostatic_eq_CF_peos_lecc_irrot(potf,potut,potux,potuy,potuz)
...
end subroutine iter_lecc_irrot_BNS_CF_mpt
