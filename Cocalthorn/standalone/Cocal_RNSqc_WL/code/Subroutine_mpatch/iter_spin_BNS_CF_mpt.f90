subroutine iter_spin_BNS_CF_mpt(iter_count, iseq, iter_eps)
...
!
  ...
  use interface_sourceterm_HaC_CF_peos_spin
  use interface_sourceterm_HaC_CF
  use interface_sourceterm_trG_CF_peos_spin
  use interface_sourceterm_trG_CF_helical
  use interface_sourceterm_MoC_CF_with_divshift_peos_spin
...
!  use interface_violation_gridpoint_HaC_CF_peos_spin
  use interface_hydrostatic_eq_CF_peos_spin
  use interface_calc_gradvep
  use interface_hydro_spbns_vep_CF_peos
  use interface_calc_surface
  use interface_update_matter
  use interface_update_parameter_BNS_spin
...
! ---
      ...
      call copy_def_matter_spin_from_mpt(impt)
! ---
...
!      emdg = 0.0d0
      if (impt==1 .or. impt==2) then
        cocp = 'ns'
        call  update_spin_BNS(iter_count)

        call interpo_fl2gr(emd, emdg)
        call interpo_fl2gr(vepxf, vepxg)
        call interpo_fl2gr(vepyf, vepyg)
        call interpo_fl2gr(vepzf, vepzg)
        call interpo_fl2gr(wxspf, wxspg)
        call interpo_fl2gr(wyspf, wyspg)
        call interpo_fl2gr(wzspf, wzspg)
!       ********************************   CHECK   ***************************************************
...
!       *********************************************************************************************
        ...
        call copy_def_matter_spin_to_mpt(impt)
...
! --- psi
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_HaC_CF_peos_spin(sou)
      else
...
!        call outer_boundary_d_BBH_CF(sou_outsurf,'psi ')
      ...
      if ((impt==1 .or. impt==2) .and. (iter_count>5 .or. iseq>1 .or. indata_type.eq.'3D')) &
         &       call update_parameter_BNS_spin(conv_gra)
...
! --- alps
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_trG_CF_peos_spin(sou)
      else
...
! --- shift
      ...
      if (impt==1 .or. impt==2) then
        call sourceterm_MoC_CF_with_divshift_peos_spin(souvec)
      else
...
!      if (error_emd_last.le.eps_coc*0.0001d0 .and. error_metr.gt.eps_coc*2.0d0) then
      if (error_emd_last.le.eps_coc*0.0001d0 .and. error_vep_last.le.eps_coc*0.0001d0 ) then
        write(6,'(a14)')  '#SKIP HYDRO b#'
      else
        if (impt==1 .or. impt==2) then
          do ihy = 1, hydro_iter
            call interpo_gr2fl_metric_CF
            call hydrostatic_eq_CF_peos_spin(potf,potut,potux,potuy,potuz)
...
!            call update_matter(potuz,vepzf,conv_den)
            ...
            call update_parameter_BNS_spin(conv_den)
            call calc_vector_x_matter(2)
            call calc_vector_phi_matter(2)
!
            call hydro_spbns_vep_CF_peos(potf,iter_count,impt,10)
            vep_bak = vep
            call update_matter(potf,vep,conv_vep)
            call reset_fluid_vep
            call calc_gradvep(vep,vepxf,vepyf,vepzf)
            call reset_fluid_gradvep
            call update_parameter_BNS_spin(conv_vep)
...
!
      if(impt>5) then
        if (mod(iter_count,5)==0.or.iter_count==1) then
          ...
          call printout_axis_spin_BNS_mpt(impt,char3)
        endif
...
!            call copy_def_matter_spin_from_mpt(jj)
!            call IO_output_solution_3D_CF_spin_NS_mpt(jj,'.mid')
...
end subroutine iter_spin_BNS_CF_mpt
