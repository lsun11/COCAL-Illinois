subroutine calc_circ_surf_peos
  ...
  use interface_source_circ_surf_peos
  ...
  integer ::  irg, irs, ii, ia, ib, iphi
!
  write(6,'(a90)') "---------------------------------------------------------------------------------------------"
  do ia = nrf,nrf  
    ib = ntfeq    ! equatorial plane
    call alloc_array2d(souf_circ_surf_ph_peos, 1, ia, 1, npf)
    call source_circ_surf_peos("ph", ia, ib, souf_circ_surf_ph_peos)
...
!  write(6,'(a90)') "---------------------------------------------------------------------------------------------"
  do ia = nrf,nrf  
    ...
    call source_circ_surf_peos("th", ia, ib, souf_circ_surf_th_peos)
...
! Output all meridional circulations
  open(12,file='circ_surf.dat', status='unknown')
  do iphi=0,npf/2
    do ia = nrf,nrf
...
end subroutine calc_circ_surf_peos
