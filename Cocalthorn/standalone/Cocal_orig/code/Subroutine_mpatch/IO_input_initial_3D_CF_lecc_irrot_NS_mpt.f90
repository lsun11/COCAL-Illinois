subroutine IO_input_initial_3D_CF_lecc_irrot_NS_mpt(impt)
  use phys_constant, only : long, nnrg, nntg, nnpg
  use def_metric
  use def_matter, only : emd, rs, vep
  use def_matter_parameter, only : ome, ber, radi, delome, delvel, velx
...
!
    ...
    open(14,file='eccentricity_mpt'//np(impt)//'.ini',status='old')
    read(14,'(1p,2e20.12)')  ome, velx
    read(14,'(1p,2e20.12)')  delome, delvel
    ome  = ome  + delome*radi
    velx = velx + delvel/radi
    close(14)
  end if
!
! --- Metric potentials.
...
end subroutine IO_input_initial_3D_CF_lecc_irrot_NS_mpt
