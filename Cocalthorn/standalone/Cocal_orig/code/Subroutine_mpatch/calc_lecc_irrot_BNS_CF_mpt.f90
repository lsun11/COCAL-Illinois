subroutine calc_lecc_irrot_BNS_CF_mpt(total_iteration)
...
!
  ...
  iter_eps = 1.0d-06
  call iter_lecc_irrot_BNS_CF_mpt(iter_count,1, iter_eps)     !  1 for freezing hydro in first 5 iterations
...
!  xb = xa - 0.25d0*xa
  ...
  iter_eps = 1.0d-06
  call iter_lecc_irrot_BNS_CF_mpt(iter_count,2, iter_eps)     ! 2 to compute hydro from first iteration
  total_iteration = total_iteration + iter_count
  write(6,'(a21,i5)') '--- total iteration =', total_iteration
  call calc_physical_quantities_irrot_BNS_CF_mpt
  call write_last_physq_BNS_mpt
...
!    write (6,'(a7,i2,a20,e20.12)') 'cycle =', icycle, '        New emdc  = ',emdc
    call iter_lecc_irrot_BNS_CF_mpt(iter_count,icycle,iter_eps)
    total_iteration = total_iteration + iter_count
    write(6,'(a21,i5)') '--- total iteration =', total_iteration
    call calc_physical_quantities_irrot_BNS_CF_mpt
    call write_last_physq_BNS_mpt
...
end subroutine calc_lecc_irrot_BNS_CF_mpt
