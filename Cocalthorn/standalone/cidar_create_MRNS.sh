...
####################### Change the following 2 paths accordingly ############
#
#path_to_src=~/COCAL_ILLINOIS/Cocalthorn
#path_to_lib=~/COCAL_ILLINOIS/lib
path_to_src=/home/antonios/repositories/cocal_illinois/Cocalthorn
path_to_lib=/home/antonios/repositories/cocal_illinois/lib
#############################################################################
...
if [ ${compact_object} == BNS ]; then
  ...
  sh select_calc_surface_quad.sh
  sh select_peos_qc.sh
elif [ ${compact_object} == MRNS ]; then
  echo "Compact object is MRNS in waveless formalism."
  sh select_grgrad_midpoint_r3rd_NS.sh
  sh select_grgrad_gridpoint_4th_NS.sh
  sh select_weight_calc_midpoint_grav_th4th.sh
  sh select_reset_fluid_radius_le_1.sh
  ...
  echo "Flag ${compact_object} should be one of {BNS,BNSqc,BBH,BHT,RNS,RNS_WL,RNSqc,RNSqc_WL,MRNS}...exiting"
  exit 1
fi
...
/bin/cp -f  coc2pri_mrs.f90        ${path_to_src}/src/${new_dir_name}/Main_utility/
...
echo "Compiling code..."
${FCOMP} ${FFLAGS} -c coc2pri_mrs.f90;   rm *.mod

echo "Creating library..."
ar rcvf libcocalmrns.a  coc2pri_mrs.o

rm coc2pri_mrs.o

cp libcocalmrns.a  ${path_to_lib}
echo "Copied library libcocalmrns.a" to ${path_to_lib}
ENDTIME=$(date +%s)
echo "----------------------------------------------------------------"
